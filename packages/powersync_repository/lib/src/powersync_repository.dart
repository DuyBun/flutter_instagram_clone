...
/// The signature to get the environmental variables based on current
/// flavor with provided [Env]
typedef EnvValue = String Function(EnumEnv env);
...
class SupabaseConnector extends PowerSyncBackendConnector {
  ...
  Future<PowerSyncCredentials?> fetchCredentials() async {
    ...
    String endP = env(EnumEnv.powerSyncUrl);
    return PowerSyncCredentials(
      endpoint: env(EnumEnv.powerSyncUrl),
      token: token,
      userId: userId,
      expiresAt: expiresAt,
    );
...
class PowerSyncRepository {
  ...
  Future<void> _loadSupabase() async {
    await Supabase.initialize(
      url: env(EnumEnv.supabaseUrl),
      anonKey: env(EnumEnv.supabaseAnonKey),
